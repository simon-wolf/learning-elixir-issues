defmodule Issues.TableFormatter do
  ...
  @doc """
  Takes a list of headers and a list of row data where each row is a Map.
  
  Prints a table of the data to STDOUT.
  
  The headers identify the data elements which are extracted from each row.
  
  The width of each column is calculated from the contents of that column.
  """
  def print_table_for_columns(rows, headers) do
    with data_by_columns = split_into_columns(rows, headers), column_widths = widths_of(data_by_columns), format = format_for(column_widths) do
  ...
  @doc """
  Given a list containing rows of data, a list containing a header selectors,
